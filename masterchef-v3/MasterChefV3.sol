...
import "./interfaces/IMorphexV3Pool.sol";
...
contract MasterChefV3 is INonfungiblePositionManagerStruct, Multicall, Ownable, ReentrancyGuard, Enumerable {
    ...
    struct UserPositionInfo {
        ...
        int24 tickLower;
        int24 tickUpper;
        uint256[10] rewardsGrowthInside;
        uint256[10] rewards;
        address user;
        uint256 pid;
    ...
    uint256 public immutable rewardTokensLength;
    /// @notice Addresses of Reward Tokens.
    IERC20[10] public rewardTokens;
    ...
    uint256[10] public latestPeriodsRewardsPerSecond;
    ...
    /// @notice Record the reward amounts belong to MasterChefV3.
    uint256[10] public rewardsAmountsBelongToMC;
    ...
    error IndexNotCorrect(uint256 index);

    event AddPool(uint256 indexed pid, uint256 allocPoint, IMorphexV3Pool indexed v3Pool, ILMPool indexed lmPool);
    ...
    event Harvest(address indexed sender, address to, uint256 indexed pid, uint256 indexed tokenId, IERC20 token, uint256 reward);
    ...
    event RewardsPerSecondUpdated(uint256 indexed index, uint256 value);
    ...
    /// @param _rewardTokens Reward tokens addresses.
    /// @param _nonfungiblePositionManager the NFT position manager contract address.
    constructor(IERC20[] memory _rewardTokens, INonfungiblePositionManager _nonfungiblePositionManager, address _WETH) {
        ...
        for (uint256 i; i < _rewardTokens.length; i++) {
            require(address(_rewardTokens[i]) != address(0), "Reward token address is zero");
    ...
    /// @notice Returns the rewards per second , period end time.
    /// @param _pid The pool pid.
    /// @return rewardsPerSecond reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfoByPid(uint256 _pid) public view returns (uint256[10] memory rewardsPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            for (uint256 i; i < 10; i++) {
                rewardsPerSecond[i] = (latestPeriodsRewardsPerSecond[i] * poolInfo[_pid].allocPoint) / totalAllocPoint;    
            }
        }
        endTime = latestPeriodEndTime;
    }  

    /// @notice Returns the rewards per second , period end time. This is for liquidity mining pool.
    /// @param _v3Pool Address of the V3 pool.
    /// @return rewardsPerSecond reward per second.
    /// @return endTime Period end time.
    function getLatestPeriodInfo(address _v3Pool) public view returns (uint256[10] memory rewardsPerSecond, uint256 endTime) {
        if (totalAllocPoint > 0) {
            for (uint256 i; i < 10; i++) {
                rewardsPerSecond[i] = (latestPeriodsRewardsPerSecond[i] * poolInfo[v3PoolAddressPid[_v3Pool]].allocPoint) / totalAllocPoint;    
            }
        }   
        endTime = latestPeriodEndTime;
    }
    ...
    /// @dev The pending rewards amount is based on the last state in LMPool. The actual amount will happen whenever liquidity changes or harvest.
    /// @param _tokenId Token Id of NFT.
    /// @return rewards Pending reward.
    function pendingRewards(uint256 _tokenId) external view returns (uint256[10] memory rewards) {
        ...
        if (positionInfo.pid != 0) {
            ...
            if (address(LMPool) != address(0)) {
                uint256[10] memory rewardsGrowthInside = LMPool.getRewardsGrowthInside(
                ...
                for (uint256 i; i < 10; i++) {
                    ...
                    unchecked {
                        rewardGrowthInsideDelta = rewardsGrowthInside[i] - positionInfo.rewardsGrowthInside[i];
                    }
                    rewards[i] = (rewardGrowthInsideDelta * positionInfo.boostLiquidity) / Q128;
                }
            }
            for (uint256 i; i < 10; i++) {
                rewards[i] += positionInfo.rewards[i];
            }
        }
    }
    ...
    function getUserInfoRewardsGrowthInside(uint256 _tokenId) external view returns (uint256[10] memory) {
        return userPositionInfos[_tokenId].rewardsGrowthInside;
    }

    function getUserInfoRewards(uint256 _tokenId) external view returns (uint256[10] memory) {
        return userPositionInfos[_tokenId].rewards;
    }
    ...
    function setReceiver(address _receiver) external onlyOwner {
        ...
        uint256 length = rewardTokensLength;
        for (uint256 i; i < length; i++) {
            if (rewardTokens[i].allowance(_receiver, address(this)) != type(uint256).max) revert();
        }
        receiver = _receiver;
        emit NewReceiver(_receiver);
    }

    function setRewardsPerSecond(uint256 _index, uint256 _value) external onlyOwner returns (bool) {
        if (_index > rewardTokensLength - 1) revert IndexNotCorrect(_index);
        latestPeriodsRewardsPerSecond[_index] = _value;
        emit RewardsPerSecondUpdated(_index, _value);
        return true;
    }
    ...
    function add(uint256 _allocPoint, IMorphexV3Pool _v3Pool, bool _withUpdate) external onlyOwner {
    ...
    /// @notice Update the given pool's Rewards allocation point. Can only be called by the owner.
    ...
    function set(uint256 _pid, uint256 _allocPoint, bool _withUpdate) external onlyOwner onlyValidPid(_pid) {
        ...
        ILMPool LMPool = ILMPool(pool.v3Pool.lmPool());
        if (address(LMPool) != address(0)) {
            LMPool.accumulateRewards(currentTime);
        }
    ...
    function onERC721Received(
        ...
        if (msg.sender != address(nonfungiblePositionManager)) revert NotMorphexNFT();
        ...
        LMPool.accumulateRewards(uint32(block.timestamp));
    ...
    /// @notice harvest rewards from pool.
    /// @param _tokenId Token Id of NFT.
    /// @param _to Address to.
    /// @return rewards Rewards.
    function harvest(uint256 _tokenId, address _to) external nonReentrant returns (uint256[] memory rewards) {
        UserPositionInfo storage positionInfo = userPositionInfos[_tokenId];
        if (positionInfo.user != msg.sender) revert NotOwner();
        uint256 length = rewardTokens.length;
        bool nonZeroRewards;
        for (uint256 i; i < length; i++) {
            nonZeroRewards = nonZeroRewards || (positionInfo.rewards[i] != 0);
        }
        if (positionInfo.liquidity == 0 && !nonZeroRewards) revert NoLiquidity();
        rewards = harvestOperation(positionInfo, _tokenId, _to);
    }
    ...
    function harvestOperation(
        ...
        if (address(LMPool) != address(0) && !emergency) {
            ...
            for (uint256 i; i < length; i++) {
                ...
                rewards[i] = (rewardGrowthInsideDelta * positionInfo.boostLiquidity) / Q128;
                positionInfo.rewardsGrowthInside[i] = rewardsGrowthInside[i];
            }
        }
        for (uint256 i; i < length; i++) {
            rewards[i] += positionInfo.rewards[i];
            if (rewards[i] > 0) {
                if (_to != address(0)) {
                    positionInfo.rewards[i] = 0;
                    _safeTransferRewardsByIndex(i, _to, rewards[i]);
                    emit Harvest(msg.sender, _to, positionInfo.pid, _tokenId, rewardTokens[i], rewards[i]);
                } else {
                    positionInfo.rewards[i] = rewards[i];
                }
            }
        }
    }
    ...
    /// @return rewards  Rewards.
    function withdraw(uint256 _tokenId, address _to) external nonReentrant returns (uint256[] memory rewards) {
    ...
    function updateLiquidityOperation(
        ...
        if (liquidityDelta != 0) {
            ...
            positionInfo.rewardsGrowthInside = LMPool.getRewardsGrowthInside(tickLower, tickUpper);
    ...
    function transferToken(address _token, address _to) internal {
        ...
        // Need to reduce rewardsAmountsBelongToMC.
        uint256 length = rewardTokensLength;
        for (uint256 i; i < length; i++) {
            if (_token == address(rewardTokens[i])) {
                unchecked {
                    // In fact balance should always be greater than or equal to rewardsAmountsBelongToMC, but in order to avoid any unknown issue, we added this check.
                    if (balance >= rewardsAmountsBelongToMC[i]) {
                        balance -= rewardsAmountsBelongToMC[i];
                    } else {
                        // This should never happend.
                        rewardsAmountsBelongToMC[i] = balance;
                        balance = 0;
                    }
                }
            }
    ...
    function sweepToken(address token, uint256 amountMinimum, address recipient) external nonReentrant {
        ...
        uint256 length = rewardTokensLength;
        for (uint256 i; i < length; i++) {
            if (token == address(rewardTokens[i])) {
                unchecked {
                    // In fact balance should always be greater than or equal to rewardsAmountsBelongToMC, but in order to avoid any unknown issue, we added this check.
                    if (balanceToken >= rewardsAmountsBelongToMC[i]) {
                        balanceToken -= rewardsAmountsBelongToMC[i];
                    } else {
                        // This should never happend.
                        rewardsAmountsBelongToMC[i] = balanceToken;
                        balanceToken = 0;
                    }
                }
            }
    ...
    function burn(uint256 _tokenId) external nonReentrant {
        ...
        if (nonZeroRewards || positionInfo.liquidity > 0) revert NotEmpty();
        delete userPositionInfos[_tokenId];
        // Update Enumerable
        removeToken(msg.sender, _tokenId);
        // Remove boosted token id in farm booster.
    ...
    /// @param _amounts The amounts of rewards injected.
    /// @param _duration The period duration.
    /// @param _withUpdate Whether call "massUpdatePools" operation.
    function upkeep(uint256[] memory _amounts, uint256 _duration, bool _withUpdate) external onlyReceiver {
        uint256 length = rewardTokensLength;
        require(_amounts.length == length, "Amounts length not correct");
        ...
        // Transfer reward tokens from receiver.
        for (uint256 i; i < length; i++) {
            rewardTokens[i].safeTransferFrom(receiver, address(this), _amounts[i]);
            // Update rewardsAmountsBelongToMC
            unchecked {
                rewardsAmountsBelongToMC[i] += _amounts[i];
            }
        }
        if (_withUpdate) massUpdatePools();
        ...
        for (uint256 i; i < length; i++) {
            uint256 rewardPerSecond;
            uint256 rewardAmount = _amounts[i];
            if (latestPeriodEndTime > currentTime) {
                uint256 remainingReward = ((latestPeriodEndTime - currentTime) * latestPeriodsRewardsPerSecond[i]) / PRECISION;
                emit UpdateUpkeepPeriod(latestPeriodNumber, latestPeriodEndTime, currentTime, address(rewardTokens[i]), remainingReward);
                rewardAmount += remainingReward;
            }
            rewardPerSecond = (rewardAmount * PRECISION) / duration;
            latestPeriodsRewardsPerSecond[i] = rewardPerSecond;
            emit NewUpkeepPeriod(latestPeriodNumber + 1, currentTime + 1, endTime, address(rewardTokens[i]), rewardPerSecond, rewardAmount);
        }
        unchecked {
            latestPeriodNumber++;
    ...
    /// @notice Update rewards for all the liquidity mining pool.
    ...
    /// @notice Update rewards for the liquidity mining pool.
    ...
    /// @param _amount Transfer Reward token amounts.
    function _safeTransferRewardsByIndex(uint256 _index, address _to, uint256 _amount) internal {
        IERC20 rewardToken = rewardTokens[_index];
        if (_amount > 0) {
            ...
            // Update rewardsAmountsBelongToMC
            unchecked {
                if (rewardsAmountsBelongToMC[_index] >= _amount) {
                    rewardsAmountsBelongToMC[_index] -= _amount;
                } else {
                    rewardsAmountsBelongToMC[_index] = balance - _amount;
                }
            }
