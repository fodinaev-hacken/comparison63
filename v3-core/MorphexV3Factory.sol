...
import './interfaces/IMorphexV3Factory.sol';
import "./interfaces/IMorphexV3PoolDeployer.sol";
import './interfaces/IMorphexV3Pool.sol';

/// @title Canonical MorphexSwap V3 factory
/// @notice Deploys MorphexSwap V3 pools and manages ownership and control over pool protocol fees
contract MorphexV3Factory is IMorphexV3Factory {
    using EnumerableSet for EnumerableSet.AddressSet;
    /// @inheritdoc IMorphexV3Factory
    ...
    EnumerableSet.AddressSet private _lmPoolDeployers;
    ...
    modifier onlyOwnerOrLmPoolDeployer() {
        require(msg.sender == owner || _lmPoolDeployers.contains(msg.sender), "Not owner or LM pool deployer");
        _;
    }
    ...
    function createPool(
        ...
        pool = IMorphexV3PoolDeployer(poolDeployer).deploy(address(this), token0, token1, fee, tickSpacing);
    ...
    function setLmPoolDeployer(address lmPoolDeployer_) external override onlyOwner {
        if (_lmPoolDeployers.add(lmPoolDeployer_)) emit SetLmPoolDeployer(lmPoolDeployer_);
    }

    function removeLmPoolDeployer(address lmPoolDeployer_) external override onlyOwner {
        if (_lmPoolDeployers.remove(lmPoolDeployer_)) emit RemovedLmPoolDeployer(lmPoolDeployer_);
    }

    function setFeeProtocol(address pool, uint32 feeProtocol0, uint32 feeProtocol1) external override onlyOwner {
        IMorphexV3Pool(pool).setFeeProtocol(feeProtocol0, feeProtocol1);
    }
    ...
    function collectProtocol(
        ...
        return IMorphexV3Pool(pool).collectProtocol(recipient, amount0Requested, amount1Requested);
    }

    function setLmPool(address pool, address lmPool) external override onlyOwnerOrLmPoolDeployer {
        IMorphexV3Pool(pool).setLmPool(lmPool);
    }
    
     function lmPoolDeployersCount() external view returns (uint256) {
        return _lmPoolDeployers.length();
    }
    
    function lmPoolDeployers(uint256 index) external view returns (address) {
        return _lmPoolDeployers.at(index);
    }
    
    function lmPoolDeployersContains(address deployer) external view returns (bool) {
        return _lmPoolDeployers.contains(deployer);
    }
}
